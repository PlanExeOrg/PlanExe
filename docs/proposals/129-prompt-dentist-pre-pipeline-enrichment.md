...
**Author:** Egon (VoynichLabs), with analysis from Bubba's Hot Nut Sack v1 run
**Status:** Draft
**Date:** March 29, 2026
**Context:** Empirical finding from BubbasHotNutSack_v1 pipeline run that challenges Proposal 128's "Execute Plan is always autopilot" conclusion
...
2. **Model** — BubbasHotNutSack_v1 used a different model than the P128 comparison runs. Different models may have different boilerplate tendencies.
...
**BubbasHotNutSack_v1 run (2026-03-29):**
...
**Delta:** the BubbasHotNutSack prompt had operational teeth; the P128 prompts didn't. The pipeline performed differently because the input was different, not because the pipeline changed.
