...
- UUID leakage into free-text fields. Models copy UUIDs from the prompt
  ...
  from full_lever_context_str (PR #457), (2) moving the UUID to the end
  of each lever's details as "(internal reference: {uuid})" so models
  are less likely to copy it. Do NOT use negative prohibitions naming
  "UUID" or "Lever ID" — small models treat the prohibition as a
  template and copy the banned phrases. Use positive framing instead.
